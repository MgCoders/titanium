...
function angle_logo() {
    $info = angle_get_logo_information();
    ...
    if ( get_theme_mod( 'logo-retina-ready', angle_get_default( 'logo-retina-ready' ) ) ) {
        $width /= 2;
        $height /= 2;
    }
 ...
 */
function angle_has_logo() {
    $information = angle_get_logo_information();
 ...
 */
function angle_get_logo_information( $force = false ) {
    $logo_information = array();

    $logo_information['image'] = get_theme_mod( 'logo', angle_get_default( 'logo' ) );

    if ( ! empty( $logo_information['image'] ) ) {
        $dimensions = angle_get_logo_dimensions( $logo_information['image'], $force );
...
function angle_get_logo_dimensions( $url, $force = false ) {
    // Build the cache key
    ...
    if ( false === $dimensions || is_preview() || true === $force ) {
        ...
        $attachment_id = angle_get_attachment_id_from_url( $url );
 ...
 */
function angle_get_attachment_id_from_url( $url = '' ) {
 ...
 * @return void
 */
function angle_display_favicons() {
    $logo_favicon = get_theme_mod( 'logo-favicon', angle_get_default( 'logo-favicon' ) );
    if ( ! empty( $logo_favicon ) ) : ?>
        <link rel="icon" href="<?php echo esc_url( $logo_favicon ); ?>"/>
    <?php endif;
}

add_action( 'wp_head', 'angle_display_favicons' );
