...
if ( ! function_exists( 'angle_font_get_relative_sizes' ) ) :
    /**
     ...
     */
    function angle_font_get_relative_sizes() {
        // Relative font sizes
        return apply_filters( 'angle_font_relative_size', array(

        ) );
    }
endif;

if ( ! function_exists( 'angle_css_fonts' ) ) :
    /**
     * Build the CSS rules for the c    ustom fonts
     *
     * @return void
     */
    function angle_css_fonts() {
        // Get relative sizes
        $percent = angle_font_get_relative_sizes();
        ...
        $declarations = angle_parse_font_properties( $element );
        if ( ! empty( $declarations ) ) {
            angle_get_css()->add( array( 'selectors' => $selectors, 'declarations' => $declarations, ) );
        }
         ...
         */
        $element = 'nav';
        $selectors = array( '.navbar-nav', '.navbar-nav a' );
        $declarations = angle_parse_font_properties( $element );
        if ( ! empty( $declarations ) ) {
            angle_get_css()->add( array( 'selectors' => $selectors, 'declarations' => $declarations, ) );
        }
        ...
        $selectors = array( '.slides .slide-content > h3', '.slides .slide-content > h3 a' );
        $declarations = angle_parse_font_properties( $element );
        if ( ! empty( $declarations ) ) {
            angle_get_css()->add( array( 'selectors' => $selectors, 'declarations' => $declarations, ) );
        }
        ...
        $selectors = array( '.single .page-header.has-post-cover h1.archive-title', '.single .page-header.has-post-cover h2.archive-title' );
        $declarations = angle_parse_font_properties( $element );
        if ( ! empty( $declarations ) ) {
            angle_get_css()->add( array( 'selectors' => $selectors, 'declarations' => $declarations, ) );
        }
         ...
         */
        $element = 'page-title';
        $selectors = array( '.page .page-header.has-post-cover h1.archive-title', '.page .page-header.has-post-cover h2.archive-title' );
        $declarations = angle_parse_font_properties( $element );
        if ( ! empty( $declarations ) ) {
            angle_get_css()->add( array( 'selectors' => $selectors, 'declarations' => $declarations, ) );
        }
...
add_action( 'angle_css', 'angle_css_fonts' );

if ( ! function_exists( 'angle_parse_font_properties' ) ) :
    /**
     ...
     */
    function angle_parse_font_properties( $element ) {
        // css_property => sanitize_callback
        $properties = apply_filters( 'angle_css_font_properties', array(
            'font-family'	=> 'angle_get_font_stack',
            'font-size'		=> 'absint',
        ), $element );

        $declarations = array();
        foreach ( $properties as $property => $callback ) {
            $value = get_theme_mod( $property . '-' . $element, angle_get_default( $property . '-' . $element ) );
            if ( false !== $value && $value !== angle_get_default( $property . '-' . $element ) ) {
                ...
                if ( 'font-size' === $property ) {
                    ...
                    // $declarations[$property . '-rem'] = angle_convert_px_to_rem( $sanitized_value ) . 'rem';
                } else {
...
if ( ! function_exists( 'angle_get_font_stack' ) ) :
    /**
     ...
     */
    function angle_get_font_stack( $font ) {
        $all_fonts = angle_get_all_fonts();

        // Sanitize font choice
        $font = angle_sanitize_font_choice( $font );

        // Standard font
        if ( isset( $all_fonts[ $font ]['stack'] ) && ! empty( $all_fonts[ $font ]['stack'] ) ) {
            $stack = $all_fonts[ $font ]['stack'];
        } elseif ( in_array( $font, angle_all_font_choices() ) ) {
         ...
         */
        return apply_filters( 'angle_font_stack', $stack, $font );
    }
endif;

if ( ! function_exists( 'angle_get_relative_font_size' ) ) :
    /**
     ...
     */
    function angle_get_relative_font_size( $value, $percentage ) {
        return round( (float) $value * ( $percentage / 100 ) );
    }
endif;

if ( ! function_exists( 'angle_convert_px_to_rem' ) ) :
    /**
     ...
     * @return float             The converted value.
     */
    function angle_convert_px_to_rem( $px, $base = 0 ) {
        return (float) $px / 10;
    }
endif;

if ( ! function_exists( 'angle_get_font_property_option_keys' ) ) :
    /**
     ...
     */
    function angle_get_font_property_option_keys( $property ) {
        $all_keys = array_keys( angle_option_defaults() );
...
if ( ! function_exists( 'angle_get_google_font_uri' ) ) :
    ...
    function angle_get_google_font_uri() {
        ...
        foreach ( $font_keys as $key ) {
            $fonts[] = get_theme_mod( $key, angle_get_default( $key ) );
        }

        // De-dupe the fonts
        $fonts         = array_unique( $fonts );
        $allowed_fonts = angle_get_google_fonts();
        $family        = array();
        ...
        foreach ( $fonts as $font ) {
            ...
            if ( array_key_exists( $font, $allowed_fonts ) ) {
                ...
                $family[] = urlencode( $font . ':' . join( ',', angle_choose_google_font_variants( $font, $allowed_fonts[ $font ]['variants'] ) ) );
            }
        }
        ...
        $subset = get_theme_mod( 'font-subset', angle_get_default( 'font-subset' ) );

        if ( 'all' === $subset ) {
            $subsets_available = angle_get_google_font_subsets();
...
if ( ! function_exists( 'angle_choose_google_font_variants' ) ) :
    /**
     ...
     */
    function angle_choose_google_font_variants( $font, $variants = array() ) {
        $chosen_variants = array();
        if ( empty( $variants ) ) {
            $fonts = angle_get_google_fonts();
         ...
         * @param array     $variants    The variants for the font.
         */
        return apply_filters( 'angle_font_variants', array_unique( $chosen_variants ), $font, $variants );
    }
endif;

if ( ! function_exists( 'angle_sanitize_font_subset' ) ) :
    /**
     ...
     */
    function angle_sanitize_font_subset( $value ) {
        if ( ! array_key_exists( $value, angle_get_google_font_subsets() ) ) {
            $value = 'latin';
        }
         ...
         */
        return apply_filters( 'angle_sanitize_font_subset', $value );
    }
endif;

if ( ! function_exists( 'angle_get_google_font_subsets' ) ) :
    /**
     ...
     */
    function angle_get_google_font_subsets() {
        /**
         ...
         */
        return apply_filters( 'angle_get_google_font_subsets', array(
            'all'          => __( 'All', 'make' ),
...
if ( ! function_exists( 'angle_sanitize_font_choice' ) ) :
    /**
     ...
     */
    function angle_sanitize_font_choice( $value ) {
        if ( ! is_string( $value ) ) {
            // The array key is not a string, so the chosen option is not a real choice
            return '';
        } else if ( array_key_exists( $value, angle_all_font_choices() ) ) {
            return $value;
        } else {
            return '';
        }
...
if ( ! function_exists( 'angle_font_choices_placeholder' ) ) :
    /**
...
if ( ! function_exists( 'angle_all_font_choices' ) ) :
    ...
    function angle_all_font_choices() {
         ...
         */
        return apply_filters( 'angle_all_font_choices', $choices );
    }
endif;

if ( ! function_exists( 'angle_all_font_choices_js' ) ) :
    /**
     * Compile the font choices for better handling as a JSON object
     *
     * @return array
     */
    function angle_all_font_choices_js() {
        $fonts   = angle_get_all_fonts();
        $choices = array();
...
if ( ! function_exists( 'angle_get_all_fonts' ) ) :
    /**
     * Compile font options from different sources.
     *
     * @return array    All available fonts.
     */
    function angle_get_all_fonts() {
        $heading1       = array( 1 => array( 'label' => sprintf( '--- %s ---', __( 'Standard Fonts', 'make' ) ) ) );
        $standard_fonts = angle_get_standard_fonts();
        $heading2       = array( 2 => array( 'label' => sprintf( '--- %s ---', __( 'Google Fonts', 'make' ) ) ) );
        $google_fonts   = angle_get_google_fonts();
         ...
         */
        return apply_filters( 'angle_all_fonts', array_merge( $heading1, $standard_fonts, $heading2, $google_fonts ) );
    }
endif;

if ( ! function_exists( 'angle_get_standard_fonts' ) ) :
    /**
     * Return an array of standard websafe fonts.
     *
     * @return array    Standard websafe fonts.
     */
    function angle_get_standard_fonts() {
        /**
         * Allow for developers to modify the standard fonts.
         *
         * @param array    $fonts    The list of standard fonts.
         */
        return apply_filters( 'angle_get_standard_fonts', array(
            'serif' => array(
...
if ( ! function_exists( 'angle_get_google_fonts' ) ) :
    /**
     * Return an array of all available Google Fonts.
     *
     * @return array    All Google Fonts.
     */
    function angle_get_google_fonts() {
        /**
         ...
         */
        return apply_filters( 'angle_get_google_fonts', array(
            'ABeeZee' => array(
                'label'    => 'ABeeZee',
                'variants' => array(
                    'regular',
